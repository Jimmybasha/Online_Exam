...
class EmailVerificationViewModel extends Cubit<EmailVerificationState> {
  ...
  EmailVerificationViewModel(this.verifyCodeUseCase)
  ...
  Future<void> verifyPassword(String passCode) async {
    ...
    result.fold(
      (error) {
        print(
            "Verification Failed: ${error.errorMessage}"); // Debugging print statement
        emit(EmailVerificationFailureState(error.errorMessage));
      },
      (status) {
        print(
            "Verification Success: ${status['status']}"); // Debugging print statement
        emit(EmailVerificationSuccessState(status['status']));
      },
    );
  }

  static void onOtpChange(String value, int index, BuildContext context,
      String otpCode, List<FocusNode> focusNodes) {
